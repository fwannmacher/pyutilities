...
class PropertyAttacher:
	@staticmethod
	def attach_property(cls, property_name, property_type = None, attribute_value_loader = None):
		"""
		...
		def loader(self):
			setattr(self, attribute, attribute_value_loader(self))
		...
		if attribute_value_loader is None:
			setattr(cls, property_name, property(getter, setter if property_type is None else type_safe_setter, deleter))
		else:
			setattr(cls, property_name, decorator.lazy_property(loader, getter, setter if property_type is None else type_safe_setter, deleter))
